...
module MechanizedAuthorization
  class << self
    def client(scope: %i[friends groups])
      @client ||= begin
        agent = Mechanize.new
        ...
        token = agent.page.uri.fragment.split("&").each_with_object({}) do |pair, hash|
    ...
    def off?
      ENV["NO_AUTH"] || !File.exist?(credentials_path)
    end
    ...
    def credentials_path
      File.expand_path("credentials.yml", __dir__)
    end
  end
end
